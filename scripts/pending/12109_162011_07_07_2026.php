<?php // scripts/pending/12109_162011_07_07_2026.php
// PROJ 12109 — RP AREA 5 PERIMETER FENCE 2.74LM 1ST BATCH-25 SPANS (org 162011)
// Fix: soft-delete 1 duplicate PR closure (WPCL-ACPR0951, ₱83,228.82).
//
...
//   25654 WPCL-ACPR0966 (2025-12-29)  ← KEEP (legit)
//   26190 WPCL-ACPR0951 (2025-12-02)  ← SOFT-DELETE (duplicate)
// Effect: WIP variance −₱83,228.82 → ₱0.00 on subacct 37263 / acct 12502 / org 162011.
// Project stays CLOSED — SAERP guards prevent future duplicates on this project.
...
return function ($cmd) {
    ...
    $WIP_SUB = 37263;
    $ORG_ID  = 162011;
    $EXPECTED_VARIANCE = -83228.82;

    // [closure_id, expected_amt, acct_doc_id]
    $KILLS = [
        [26190, 83228.82, 103803706],
    ];
    ...
    $say($line);
    $say(' PROJ 12109 — RP AREA 5 PERIMETER FENCE (org 162011)');
    $say(' Kill 1 duplicate closure (WPCL-ACPR0951, ₱83,228.82)');
