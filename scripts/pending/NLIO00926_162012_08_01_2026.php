<?php // NLIO00926_162012_08_01_2026.php
// ============================================================================
// LMC payout headers double-book the account-pair credit — NLIO00926
// Org 162012 · mysql_secondary · 2026-08-01
//
//   NLIO00926 - STANDARD LAWN INT. W/O MASS (FEBRUARY 2026) has two pure
//   account-pair credit payouts. Neither has a labour payout line, yet each
//   carries its account-pair amount in amt_total_payout and
//   amt_total_payout_net, which by convention hold the labour total only.
//
//     NLMC0008367   540.00   scope 25903
//     NLMC0008377 5,105.12   scope 25903
//
//   Project Stage Variance reads the header as
//     amt_total_payout_net + amt_total_acctpair_credit_payout_net
//   so scope 25903 INTERMENT ORDER prints 23,437.99 instead of 17,792.87.
//
//   Two of 127 headers written by 'WEB Accounting' on 2026-07-27 and 07-28.
//   The eight Java-written payouts on this project are correct and untouched.
//
//   Sets each header's two labour totals to the sum of its own payout lines,
//   which is 0.00 for both. The account-pair columns, every payout line, every
//   balance and the ledger are untouched.
//
// WRITES     wip_t_lmc_payout — 2 UPDATEs, 2 columns each
// ROLLBACK   NLIO00926_162012_08_01_2026_rollback.php
// ============================================================================
return function ($cmd) {
    // ---------------- CONFIGURATION ----------------
    $APPROVED = false;                       // <-- set true only with approval on record
    ...
    // payout id => [documentno, scope, account-pair amount]
    $PAYOUTS = [
        55795 => ['NLMC0008367', 25903,  540.00],
        55809 => ['NLMC0008377', 25903, 5105.12],
    ];
    // scope => [reports now, reports after]
    $SCOPES = [
        25903 => [23437.99, 17792.87],
    ];
    ...
    $say($L);
    $say(' NLIO00926 — LMC payout headers double-book the account-pair credit');
    ...
    // ---------------- GATES 1-3 · every header, before anything is written ----------------
    $plan = [];
    foreach ($PAYOUTS as $id => [$docno, $scope, $ap]) {
        ...
        if (abs((float) $h->ap_net - $ap) > 0.001 || abs((float) $h->ap_gross - $ap) > 0.001)
            throw new \RuntimeException("GATE 1 FAILED: $docno account-pair columns are "
                . $m($h->ap_gross) . ' / ' . $m($h->ap_net) . ', expected ' . $m($ap) . '. ABORT.');
        ...
        if ($h->created !== $WRITER)
            throw new \RuntimeException("GATE 1 FAILED: $docno created='{$h->created}', expected "
                . "'$WRITER'. Only the backend-written headers carry this defect. ABORT.");
        ...
        $plan[$id] = [$docno, $scope, $ap, $lab];
        $say(sprintf('        %-8s %-13s scope %-6s header %10s / %10s · lines %6s · acctpair %10s',
             $id, $docno, $scope, $m($h->amt_total_payout), $m($h->amt_total_payout_net),
             $m($lab), $m($apl)));
    }

    // ---------------- GATE 4 · each scope reads what we audited ----------------
    foreach ($SCOPES as $scope => [$expBefore, $expAfter]) {
        $now = $reported($scope);
        if (abs($now - $expBefore) > 0.01)
            throw new \RuntimeException("GATE 4 FAILED: scope $scope reports " . $m($now)
                . ', expected ' . $m($expBefore) . '. ABORT.');
    }

    $say(' GATES  0 replica · 1 headers · 2 no labour lines · 3 account-pair intact · 4 report ..... PASS');
    $say('');
    $say('   PLAN — ' . count($plan) . ' UPDATEs on wip_t_lmc_payout');
    foreach ($plan as $id => [$docno, $scope, $ap, $lab])
        $say('   ' . $docno . '  amt_total_payout & _net  ' . $m($ap) . '  ->  ' . $m($lab));
    $say('');
    foreach ($SCOPES as $scope => [$expBefore, $expAfter])
        $say('   scope ' . $scope . ' LMC Consume  ' . $m($expBefore) . '  ->  ' . $m($expAfter));
    ...
    if (!$APPROVED) {
        ...
        $say($L);
        if (isset($cmd)) $cmd->info('NLIO00926 DRY-RUN: ' . count($plan) . ' updates planned, no writes.');
        return;
    }
    ...
    try {
        ...
        foreach ($SCOPES as $scope => [$expBefore, $expAfter]) {
            ...
            $say('             scope ' . $scope . ' LMC Consume  ' . $m($expBefore) . '  ->  ' . $m($after));
        }

        $db->commit();

        $say(' COMPLETE — reprint Project Stage Variance for NLIO00926');
        $say($L);
        if (isset($cmd)) $cmd->info('NLIO00926 fixed: ' . count($plan) . ' headers.');
